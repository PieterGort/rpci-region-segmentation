...
├── Scan_001_TS.nii.gz
├── Scan_002_TS.nii.gz
├── Segmentations_001_all.nii.gz
├── Segmentations_002_all.nii.gz
└── ...
```

Use the same case identifier in each pair: `Scan_{case_id}_TS.nii.gz` and `Segmentations_{case_id}_all.nii.gz`. If labels have been generated by the dilation pipeline, use `Segmentations_{case_id}_all_expanded.nii.gz`.
